...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
		...
		$language_items = array(
			...
			'help_account_mail',
			'help_account',
			'help_admission_order_status',
			'help_affiliate_approval',
			'help_affiliate_auto',
			'help_affiliate_commission',
			'help_affiliate_mail',
			'help_affiliate',
			'help_api',
			'help_captcha',
			...
			'help_checkout',
			'help_comment',
			'help_sales_commission1_amount',
			'help_sales_commission1_status',
			'help_sales_commission2_status',
			'help_complete_status',
			'help_compression',
			'help_currency_auto',
			'help_currency',
			'help_customer_group_display',
			'help_customer_group',
			'help_customer_online',
			'help_customer_price',
			'help_down_payment_limit',
			'help_down_payment_status',
			'help_encryption',
			...
			'help_fraud_status',
			'help_ftp_root',
			'help_full_payment_limit',
			'help_full_payment_status',
			'help_geocode',
			'help_icon',
			'help_initial_payment_limit',
			'help_initial_payment_status',
			'help_invoice_prefix',
			'help_limit_admin',
			'help_location',
			'help_lock_complete_order',
			'help_lock_printed_transaction',
			'help_login_attempts',
			'help_mail_alert',
			'help_mail_parameter',
			'help_mail_protocol',
			...
			'help_maintenance',
			'help_notification_start',
			'help_open',
			'help_order_mail',
			'help_order_status',
			'help_password',
			'help_processing_status',
			'help_reference_start',
			'help_reverse_entry_limit',
			'help_review_guest',
			'help_review_mail',
			'help_review',
			'help_robots',
			...
			'help_seo_url',
			'help_shared',
			'help_status_with_payment',
			'help_stock_checkout',
			'help_stock_display',
			'help_stock_warning',
			'help_tax_customer',
			'help_tax_default',
			'help_transaction_initial_payment',
			'help_vendor_purchase_initial',
			'help_voucher_max',
			'help_voucher_min',
			'button_cancel',
			'button_save',
		...
		// if (isset($this->error['customer_deposit'])) {
		// 	$data['error_customer_deposit'] = $this->error['customer_deposit'];
		// } else {
		// 	$data['error_customer_deposit'] = '';
		// }
		...
		if (isset($this->request->post['config_customer_deposit'])) {
			$data['config_customer_deposit'] = $this->request->post['config_customer_deposit'];
		} else {
		...
		if (isset($this->request->post['config_lock_printed_transaction'])) {
			$data['config_lock_printed_transaction'] = $this->request->post['config_lock_printed_transaction'];
		} else {
			$data['config_lock_printed_transaction'] = $this->config->get('config_lock_printed_transaction');
		}
		...
		if (isset($this->request->post['config_invoice_prefix'])) {
			...
			$data['config_invoice_prefix'] = 'INV-{YEAR}/0';
		}

		if (isset($this->request->post['config_receipt_customer_prefix'])) {
			$data['config_receipt_customer_prefix'] = $this->request->post['config_receipt_customer_prefix'];
		} elseif ($this->config->get('config_receipt_customer_prefix')) {
			$data['config_receipt_customer_prefix'] = $this->config->get('config_receipt_customer_prefix');
		} else {
			$data['config_receipt_customer_prefix'] = '';
		}

		if (isset($this->request->post['config_receipt_vendor_prefix'])) {
			$data['config_receipt_vendor_prefix'] = $this->request->post['config_receipt_vendor_prefix'];
		} elseif ($this->config->get('config_receipt_vendor_prefix')) {
			$data['config_receipt_vendor_prefix'] = $this->config->get('config_receipt_vendor_prefix');
		} else {
			$data['config_receipt_vendor_prefix'] = '';
		}

		if (isset($this->request->post['config_agreement_vendor_prefix'])) {
			$data['config_agreement_vendor_prefix'] = $this->request->post['config_agreement_vendor_prefix'];
		} elseif ($this->config->get('config_agreement_vendor_prefix')) {
			$data['config_agreement_vendor_prefix'] = $this->config->get('config_agreement_vendor_prefix');
		} else {
			$data['config_agreement_vendor_prefix'] = '';
		}

		if (isset($this->request->post['config_admission_vendor_prefix'])) {
			$data['config_admission_vendor_prefix'] = $this->request->post['config_admission_vendor_prefix'];
		} elseif ($this->config->get('config_admission_vendor_prefix')) {
		...
		if (isset($this->request->post['config_reference_start'])) {
			$data['config_reference_start'] = $this->request->post['config_reference_start'];
		} elseif ($this->config->get('config_reference_start')) {
			$data['config_reference_start'] = $this->config->get('config_reference_start');
		} else {
			$data['config_reference_start'] = '0';
		}
		...
		if (isset($this->request->post['config_sales_commission1_status'])) {
			$data['config_sales_commission1_status'] = $this->request->post['config_sales_commission1_status'];
		} elseif ($this->config->get('config_sales_commission1_status')) {
			$data['config_sales_commission1_status'] = $this->config->get('config_sales_commission1_status');
		} else {
			$data['config_sales_commission1_status'] = array();
		}

		if (isset($this->request->post['config_sales_commission2_status'])) {
			$data['config_sales_commission2_status'] = $this->request->post['config_sales_commission2_status'];
		} elseif ($this->config->get('config_sales_commission2_status')) {
			$data['config_sales_commission2_status'] = $this->config->get('config_sales_commission2_status');
		} else {
			$data['config_sales_commission2_status'] = array();
		}

		if (isset($this->request->post['config_sales_commission1_amount'])) {
			$data['config_sales_commission1_amount'] = $this->request->post['config_sales_commission1_amount'];
		} elseif ($this->config->get('config_sales_commission1_amount')) {
			$data['config_sales_commission1_amount'] = $this->config->get('config_sales_commission1_amount');
		} else {
			$data['config_sales_commission1_amount'] = '0';
		}

		if (isset($this->request->post['config_sales_commission'])) {
			$data['config_sales_commission'] = $this->request->post['config_sales_commission'];
		} elseif ($this->config->has('config_sales_commission')) {
		   	$data['config_sales_commission'] = $this->config->get('config_sales_commission');
		} else {
			$data['config_sales_commission'] = array();
		}

		$data['sales_commissions'] = [];

		$data['sales_commissions'][] = [
			'text'  => $this->language->get('text_sales_commission1'),
			'value' => 'commission1'
		];
		
		$data['sales_commissions'][] = [
			'text'  => $this->language->get('text_sales_commission2'),
			'value' => 'commission2'
		];
		...
		if (isset($this->request->post['config_notification_start'])) {
			$data['config_notification_start'] = $this->request->post['config_notification_start'];
		} else {
			$data['config_notification_start'] = $this->config->get('config_notification_start');
		}

		if (isset($this->request->post['config_initial_payment_status_id'])) {
			$data['config_initial_payment_status_id'] = $this->request->post['config_initial_payment_status_id'];
		} else {
			$data['config_initial_payment_status_id'] = $this->config->get('config_initial_payment_status_id');
		}

		if (isset($this->request->post['config_initial_payment_amount'])) {
			$data['config_initial_payment_amount'] = $this->request->post['config_initial_payment_amount'];
		} else {
			$data['config_initial_payment_amount'] = $this->config->get('config_initial_payment_amount');
		}

		if (isset($this->request->post['config_initial_payment_limit'])) {
			$data['config_initial_payment_limit'] = $this->request->post['config_initial_payment_limit'];
		} else {
			$data['config_initial_payment_limit'] = $this->config->get('config_initial_payment_limit');
		}

		if (isset($this->request->post['config_down_payment_status_id'])) {
			$data['config_down_payment_status_id'] = $this->request->post['config_down_payment_status_id'];
		} else {
			$data['config_down_payment_status_id'] = $this->config->get('config_down_payment_status_id');
		}

		if (isset($this->request->post['config_down_payment_amount'])) {
			$data['config_down_payment_amount'] = $this->request->post['config_down_payment_amount'];
		} else {
			$data['config_down_payment_amount'] = $this->config->get('config_down_payment_amount');
		}

		if (isset($this->request->post['config_down_payment_limit'])) {
			$data['config_down_payment_limit'] = $this->request->post['config_down_payment_limit'];
		} else {
			$data['config_down_payment_limit'] = $this->config->get('config_down_payment_limit');
		}

		if (isset($this->request->post['config_full_payment_status_id'])) {
			$data['config_full_payment_status_id'] = $this->request->post['config_full_payment_status_id'];
		} else {
			$data['config_full_payment_status_id'] = $this->config->get('config_full_payment_status_id');
		}

		if (isset($this->request->post['config_full_payment_limit'])) {
		...
		if (isset($this->request->post['config_expired_status_id'])) {
			$data['config_expired_status_id'] = $this->request->post['config_expired_status_id'];
		} else {
			$data['config_expired_status_id'] = $this->config->get('config_expired_status_id');
		}

		// if (isset($this->request->post['config_transaction_down_payment'])) {
			// $data['config_transaction_down_payment'] = $this->request->post['config_transaction_down_payment'];
		// } elseif ($this->config->has('config_transaction_down_payment')) {
			// $data['config_transaction_down_payment'] = $this->config->get('config_transaction_down_payment');
		// } else {
			// $data['config_transaction_down_payment'] = '30';
		// }

		if (isset($this->request->post['config_status_with_payment'])) {
			$data['config_status_with_payment'] = $this->request->post['config_status_with_payment'];
		} elseif ($this->config->get('config_status_with_payment')) {
	...
	protected function validate() {
		...
		if ($this->request->post['config_customer_deposit'] < 0) {
			$this->error['customer_deposit'] = $this->language->get('error_customer_deposit');
		}
