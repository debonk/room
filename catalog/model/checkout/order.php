...
class ModelCheckoutOrder extends Model
{
	...
	public function deleteOrder($order_id)
	{
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_document` WHERE order_id = '" . (int)$order_id . "'");
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false, $user_id = 0)
	{
		...
		if ($order_info) {
			...
			/* 			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$data['text_title'] = $language->get('text_new_title');
				...
				$text .= $data['text_title'] . ' ' . $data['title'] . "\n";
